...
const char* direct__(shell___* sh,int cmd,...){
	...
	case call4_1_:
	{
		...
		if(cmd==call4_)
			...
			ret=sh->call4_1__(err,code,shangji,zhao,dong,src2,argc,argv1,from);
		}
		break;
	}
	case on_:
...
bool shell___::plugin_proc__(std::deque<Glib::ustring>* p1,char*buf,long siz,char**addr_ret,void*shangji){
	...
	if(p1_0=="插件信息"){
		...
		for(size_t i=0;i<plugins_.size();i++){
			plugin___* p=(plugin___*)plugins_[i];
			const char*ret0=l4_.l4_callback_1__(l4_.l4__(),&err,
	...
	if(p1_0=="谁的"){
		if(p1->size()>1){
			...
			for(size_t i=0;i<plugins_.size();i++){
				plugin___* p=(plugin___*)plugins_[i];
				switch(p->plugin_->proc__(&p2,buf,-1,addr_ret,shangji)){
				case 1:
	...
	for(size_t i=0;i<plugins_.size();i++){
		plugin___* p=(plugin___*)plugins_[i];
		switch(p->plugin_->proc__(p1,buf,siz,addr_ret,shangji)){
		case 1:
			return true;
		case 2:
			proc=true;
