...
int shell___::with__(int argc, char *argv[]){
	{
		//-1
...
const char* shell___::call4_1__(int* err,const char*code,void* shangji,
	...
	if(!shangji)
		shangji=l4_main_qu_;
	return l4_.l4_callback3_1__(l4_.l4__(),err,code,false,src2,shangji,argc,argv,from);
}
...
const char* shell___::call4__(const char*code,void* shangji,
		const char*zhao,const char*dong,const char* src2,int argc,const char*argv[],int from){
	int err;
	const char*ret=call4_1__(&err,code,shangji,zhao,dong,src2,argc,argv,from);
	call4_2__(err);
	return ret;
}
...
Glib::ustring null_;
static void suidao__(char**addr_ret,char*buf,long siz,shell___* sh,void* shangji,int argc,...) {
	std::deque<Glib::ustring> p;
	_for_args( argc )
		p.push_back(s?s:null_.c_str());
	_next_args
	try{
		sh->api__(shangji,&p,buf,siz,addr_ret);
	}
	catch(int err){
...
int shell___::l4__(int argc, char *argv[],int from){
	int err;
	{
	...
	l4_main_qu_=l4_.new_main_qu__();
	{
		...
		Glib::ustring s;
		s+=dy;
		s+=ky;s+="ASZ/";s+=l2x__((long)suidao__,buf);s+="/&-&";s+=by;s+=d;
		...
		if((err=l4_.def_new__("隧道",s.c_str(),true,false,l4_main_qu_)))
			return err;
		if((err=l4_.alias_new__("□","隧道",true,l4_main_qu_)))
			return err;
	}
