...
public:
	...
	int with__(int argc, char *argv[]);
	void api__(void*shangji,std::deque<Glib::ustring>* p,char*buf,long siz,char**addr_ret);
	...
	const char* call4_1__(int* err,const char*code,void* shangji,const char*zhao,const char*dong,const char* src2,int argc,const char*argv[],int from);
...
private:
	...
	int l4__(int argc, char *argv[],int from);

	std::deque<void*>plugins_;
	void pluginit__();
	bool plugin_proc__(std::deque<Glib::ustring>* p,char*buf,long siz,char**addr_ret,void*shangji);
