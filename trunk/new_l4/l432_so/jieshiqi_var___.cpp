...
const char* callback1__(jieshiqi___* jsq,qu___* shangji,int* err,bool bylib,call_ex___* ce,
    ...
    for(int i = 0; i < argc; ++i){
        ...
        args.params_push_back__(s?s:jsq->syn_[keyword_null_]);
    }
		...
		int type,bool is_noparam)
{
	...
	if(!name||!val||!jsq/*||!qu*/)
		...
		err=jsq->rems__(buf2,&rems,&qu2,&qu2,&qu2,&readonly,&is_noparam,&is_code,&is_yuanyang);
		if(!err)
			err=jsq->var_new__(qu2,buf2,val,readonly,type,is_noparam,&rems);
	}
	if(err)
		err+=jieshiqi_err_;
	return err;
...
int jieshiqi___::rems__(list<string>* rems,qu___** top,qu___** main1,qu___** shangji,
		bool* lock,bool* is_noparam,bool* is_code,bool* is_yuanyang)
{
	if(syn_.rem_kw__(rems,keyword_top_)){
		if(top)
		*top=&top_qu_;
	}
	if(syn_.rem_kw__(rems,keyword_main_)){
		if(main1)
			*main1=main_qu__(*main1);
	}
	for(;;){
		if(!syn_.rem_kw__(rems,keyword_shangji_))
			break;
		if(shangji){
			if(!(*shangji)->shangji_)
	...
	if(syn_.rem_kw__(rems,keyword_lock_)){
		if(lock)
		*lock=true;
	}
	if(syn_.rem_kw__(rems,keyword_noparam_)){
		if(is_noparam)
		*is_noparam=true;
	}
	if(syn_.rem_kw__(rems,keyword_code_)){
		if(is_code)
		*is_code=true;
	}
	if(syn_.rem_kw__(rems,keyword_yuanyang_)){
		if(is_yuanyang)
		*is_yuanyang=true;
	}
	return 0;
...
#endif
	return rems__(rems,top,main1,shangji,lock,is_noparam,is_code,is_yuanyang);
}
...
int jieshiqi___::var__(string&buf2,string& buf,qu___* qu,int kw,bool* push_p,deque<string>* p){
	...
	/*//这不成拦截了？
	if(top_qu_.vars_.find(buf2)!=top_qu_.vars_.end()){
		switch(kw){
		case keyword_has_:
			buf+=has_true_;
			break;
		default:
			buf+=top_qu_.vars_[buf2].val__(&rems);
			break;
		}
		return 0;
	}*/
	...
	if(qu2->args_){
		...
		switch(num=kw_num__(syn_[keyword_params_],1,qu2->args_->params_.size(),buf2,NULL)){
		case kwnum_false_:
			break;
		default:
			switch(kw){
	...
	for(;;){
		...
		if(!var){
			...
			default:
				...
				if(var->type_==vartype_alias_){
					string buf3=var->val__(&rems);
					err=jieshiqi___::var__(buf3,buf,qu2,kw,push_p,p);
					if(err)
						err_<<syn_[keyword_alias_]<<buf2;
					return err;
				}else
...
int jieshiqi___::set_equ__(qu___* qu,size_t& offi,size_t to,int kw,string& buf,int kw0,deque<string>* eval_p){
	...
	//原样还得保留回车等
	bool is_yuanyang=false;
	if((err=rems__(buf2,&rems,&qu2,&qu2,&qu2,&readonly,&is_noparam,&is_code,&is_yuanyang))){
	...
	if((err=jieshi__(qu,from2,to2,buf3,NULL,NULL,kw,kwsm_1_,maskt)))
		return err;
	if((err=var_new__(qu2,buf2,buf3,readonly,type,is_noparam,&rems))){
		return err;
	}
	if(kw0!=keyword_no_)
		buf+=buf3;
		...
		list<string>* rems)
{
	...
	var->is_lib_=(qu2==&top_qu_&&var->readonly_);
	return 0;
}
