...
int err_free__(int err,unsigned long *ul,char* c){
	if(ul)
		delete ul;
	if(c)
		delete c;
...
int call_func__(deque<string>* p,string& buf0,err___* err0,const string& null,
	...
	if(func_fmt.substr(i1+1)=="@"){
		...
		unsigned long *argv=new unsigned long[p->size()+16];//S、E、A也占名额
		int argc=0;

		int err_ret=0;
		#define bufsiz2 1024
		size_t bufsiz=bufsiz2;
		char buf[bufsiz2];
		buf[0]=0;
		bool use_buf=false;
		...
		if(!(ret_type & calltype_niyazhan_)){
			if(ret_type & calltype_ret_err_)
				argv[argc++]=(unsigned long)&err_ret;
			if(ret_type & calltype_ret_addr_)
				argv[argc++]=(unsigned long)&addr_ret;
			if(ret_type & (calltype_buf_siz_|calltype_buf_))
				argv[argc++]=(unsigned long)buf;
		...
		if(!(i1==func_fmt.length()-1&&p->size()==1)){
			...
			for(;;){
				...
				if(type & calltype_err_type_)
					return err_free__(errinfo_func_type_,argv);
				switch(type){
				case calltype_buf_:
				...
				case calltype_ret_addr_:
					argv[argc++]=(long)&addr_ret;
					break;
				case calltype_ret_err_:
					argv[argc++]=(long)&err_ret;
					break;
				case calltype_var_:
				{
					switch(ret2){
					case kwnum_false_:
						switch(ret4){
						case calltype_long_:
							argv[argc++]=(unsigned long)s2l__(ret3);
							break;
						case calltype_unsigned_|calltype_long_:
							argv[argc++]=(unsigned long)s2lu__(ret3);
							break;
						case calltype_char_|calltype_zhizhen_:
							string& cbuf=cb_buf__();
							cbuf=ret3;
							argv[argc++]=(unsigned long)cbuf.c_str();
							break;
						}
						break;
					case 11:
						argv[argc++]=(unsigned long)ce;
						break;
					default:
						string& cbuf=cb_buf__();
						cbuf=callbackx__(ret2,jsq,qu);
						argv[argc++]=(unsigned long)x2l__(cbuf);
						break;
					}
					break;
				}
				default:
					{
						if(i2<1||i2>=p->size())
							return err_free__(errinfo_param_num_,argv);
						string arg=(*p)[i2];
						switch(type){
						case calltype_char_:
						case calltype_unsigned_|calltype_char_:
						...
						default:
							*err0<<func_fmt;
							return err_free__(errinfo_func_type_allow_,argv);
						}
						if(i1==string::npos)
							break;
						i2+=step;
				...
				if(i1==string::npos){
					if((i2>1&&step<0)||(i2<p->size()-1&&step>0))
						return err_free__(errinfo_param_over_,argv);
					break;
				}
			}
		}
		...
		if(err_ret){
			...
			return err_free__(err_ret,argv);
		}
		...
		case calltype_unsigned_|calltype_char_|calltype_zhizhen_:
			if(!zhizhen__(ax_qi4,buf0,err0,null))
				return err_free__(errinfo_bad_val_,argv);
			break;
		case calltype_char_|calltype_addr_:
		case calltype_unsigned_|calltype_char_|calltype_addr_:
		case calltype_addr_:
		...
		if(addr_ret){
			if(!zhizhen__((unsigned long)addr_ret,buf0,err0,null))
				return err_free__(errinfo_bad_val_,argv,addr_ret);
			delete addr_ret;
		}
		if(use_buf)
			buf0+=buf;
		return err_free__(0,argv);
	}
}
