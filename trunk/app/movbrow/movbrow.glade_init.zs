...
循环先
	赋予号以算术‘号’+1。
	如果不存在视频标示之‘号’那么跳出。
	如果调用‘串位置方法’为‘参数1’为“ ‘视频标示之‘号’’”大于0那么1，退出。
了。
0。”。
...
定义【主】扫描视频以下代码
	...
	循环先
		赋予号以算术‘号’+1。
		如果不存在视频目录之‘号’那么跳出。
		如果存在视频目录始号并且‘号’小于‘视频目录始号’那么再来。
		如果存在视频目录终号并且‘号’大于‘视频目录终号’那么跳出。
...
定义【主】扫描目录以下代码
	赋予搜以调用‘dir.begin’为‘参数1’、、‘目录控制’。调用‘dir.sort’为‘搜’为0。
	循环先
		...
		当先
			...
			那么先
				赋予类型信息以调用‘重定向输出’为“file -L '‘路径’'”。
				如果不是视频‘类型信息’那么再来。
				赋予目录以解释‘目录名’为‘路径’。
				赋予名以先调用‘加反斜杠’为先解释‘文件主名’为‘路径’了为R了。
				赋予搜图以调用‘dir.begin’为‘目录’为^‘名’\..*(png|jpg|jpeg|PNG|JPG|JPEG)$。
				赋予缩略图以调用‘dir.next’为‘搜图’。
				调用‘dir.end’为‘搜图’。
...
定义【主】测试安装以下代码
	赋予空格以调用‘串位置方法’为‘参数1’为“ ”。
	如果‘空格’大于等于0那么赋予命令名以调用‘子串方法’为‘参数1’为0、‘空格’。
	否则赋予命令名以‘参数1’。
...
定义【主】播放当前视频以下代码
	...
	上代码那么先
		...
		如果播放器选择player1那么先
			赋予命令以‘无边行头’先
				如果存在指定播放器参1那么‘指定播放器参1’否则隧道内容为p1_args
			了“ ”'‘视频’'。
			显示‘命令’↙。
			赋予【主】简播进程号以同时执行‘命令’。
			如果隧道勾选为minionplay那么
	...
	图标夹移除‘图标夹’为下代码
		...
		了否则先
			...
			如果‘图片’不等于‘需要同名图片’那么
				赋予命令以‘命令’↙“mv '‘图片’' /tmp”。
		了。
		赋予命令以‘命令’↙“rmdir -p '”先调用‘得目录名’、‘视频’了“' #如果目录为空将删除”。
...
隧道侦听为点击为info、m_info为下代码
	图标夹获取‘图标夹’为下代码
		...
		赋予建立时间以建立:→得时间c。
		赋予修改时间以修改:→得时间m。
		隧道提示为路径:→‘视频’↙↙‘大小’↙↙‘访问时间’↙‘修改时间’↙‘建立时间’为‘标题’的信息。
	上代码。
上代码。
