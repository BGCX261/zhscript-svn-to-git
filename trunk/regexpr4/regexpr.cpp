...
bool regexpreplace__(int*err,char*buf,long siz,char* src,char* from,
		char* to,void* shangji,int cflags,int eflags,string* s){
	regex_t      reg;
	int z=regcomp(&reg, from, cflags);
	if(z != 0)
    {
		...
		if(shangji){
			if(s){
				...
				for(int i2=1;i2<nmatch_;i2++){
					...
					src[eo]=0;
					sprintf(buf2,"%d",i2);
					const char* ret=callback_(jsq_,shangji,err,to,false,3,
							src+start+pm2.rm_so,
							buf2,buf1);
					*s+=ret;
					src[eo]=c_old;
				...
				callback3_(jsq_,shangji,err,to,false,argc,argv,0);
				if(*err==jieshiqi_err_go_+keyword_continue_)
					*err=0;
			}
			if(*err){
...
dlle___ void dlln___(regexpreplace__)(int*err,char**addr_ret,char*buf,long siz,
		bool fore,void* shangji,char* src,char* from,char* to,int argc,...){
	if(!src||!from||!to)
		return;
	int cflags=REG_EXTENDED,eflags=0;
	_for_args(argc)
	...
	string s;
	if(!regexpreplace__(err,buf,siz,src,from,to,shangji,cflags,eflags,fore?NULL:&s))
		return;
...
dlle___ void dlln___(regexpr__)(int*err,char**addr_ret,char*buf,long siz,int argc,...){
	...
	regex_t      reg;
	int z=regcomp(&reg, exp, REG_EXTENDED);
	if(z != 0)
    {
		get_reg_err__(z,&reg,buf,siz);
		regfree(&reg);
