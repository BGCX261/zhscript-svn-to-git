        ...
        gpointer data)
{
	...
	if ((selection_data != NULL) && (selection_data->length >= 0)) {
		...
		int err;
		cb->d_(cb->sh_,call4_1_,cb->code_.c_str(),NULL,NULL,NULL,NULL,3,argv,0,&err);
		if(err){
...
int dnd___::proc__(std::deque<Glib::ustring>* p,char*buf,long siz,char**addr_ret,void*shangji) {
	if((*p)[0]==name__()){
		if(siz==-1)return 1;
		if(p->size()<3){
